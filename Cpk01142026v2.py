...
def process_cpk(station, df):
    ...
    # [修改] 尋找含有 "config" 字眼的表頭列，用來界定數據的起始位置
    ...
    # [修改] 計算資料起始列：取各標題列的最大值 + 1
    # 若找不到 config_header_row，則預設不影響計算
    ...
    # [新增] 提取 Config 欄位 (假設在第 2 欄, index=1)
    # 如果您的 config 在其他欄位，請修改此處的 index
    config_col_idx = 1
    ...
    results = []
    # 根據 Date 和 Config 進行分組計算 CPK
    # 這樣如果同一天有 SH2 和 SH3，會被分開計算
        ...
        for idx, dim in dim_cols.items():
            ...
            if n > 0:
                results.append({
                    "Station": station,
                    "Dim No": dim,
                    "Config": config_val, # [修改] 填入該組的 config 值 (如 SH2, SH3)
                    "Date": date,
                    "Sample Size": n,
                    "USL": usls.get(idx, ""),
                    "LSL": lsls.get(idx, ""),
