...
if uploaded:
    ...
    yield_list, cpk_list, cpk_config_list = [], [], [] # 初始化列表
    ...
    df_cpk_config = pd.DataFrame(cpk_config_list) # 新增 DataFrame
    ...
    if not df_cpk.empty:
        ...
        sort_cols = ["Station", "Dim No", "Date", "Config"]
        df_cpk = df_cpk.sort_values(sort_cols)
        
    # [新增] 處理 CPK by Config 顯示
    if not df_cpk_config.empty:
        df_cpk_config["Station"] = pd.Categorical(df_cpk_config["Station"], TARGET_ORDER, ordered=True)
        # 排序不含 Date
        sort_cols_config = ["Station", "Dim No", "Config"]
        df_cpk_config = df_cpk_config.sort_values(sort_cols_config)

    # 建立三個分頁
    tab1, tab2, tab3 = st.tabs(["Yield", "CPK", "CPK by Config"])
    ...
    with tab3:
        st.write("此頁面合併所有日期，僅依據 Config 計算 CPK")
    ...
    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
        ...
        df_cpk_config.to_excel(writer, sheet_name="CPK by Config", index=False) # 新增 Sheet
        ...
        # 格式化 CPK Sheet
        ws_cpk = writer.sheets["CPK"]
        if not df_cpk.empty:
            cpk_col = excel_col_letter(df_cpk.columns.get_loc("CPK"))
            ws_cpk.conditional_format(
                f"{cpk_col}2:{cpk_col}{len(df_cpk)+1}",
                {"type": "cell", "criteria": "<", "value": CPK_LIMIT, "format": red}
            )
            
        # 格式化 CPK by Config Sheet
        ws_cpk_config = writer.sheets["CPK by Config"]
        if not df_cpk_config.empty:
            cpk_config_col = excel_col_letter(df_cpk_config.columns.get_loc("CPK"))
            ws_cpk_config.conditional_format(
                f"{cpk_config_col}2:{cpk_config_col}{len(df_cpk_config)+1}",
                {"type": "cell", "criteria": "<", "value": CPK_LIMIT, "format": red}
            )
