...
def process_cpk(station, df):
    ...
    usls, lsls, configs = {}, {}, {} # [修改] 初始化 configs
    if usl_row != -1:
    ...
    # [新增] 提取 config 列的內容 (例如 LM, inline IPQC)
    if config_row != -1:
        for i, v in enumerate(df.iloc[config_row]):
            if not pd.isna(v):
                configs[i] = str(v).strip()

    # [修改] 計算資料起始列時，也要考慮 config_row
    start = max(dim_row, usl_row, lsl_row, config_row) + 1
        ...
        for idx, dim in dim_cols.items():
            ...
            if n > 0:
                results.append({
                    "Station": station,
                    "Dim No": dim,
                    "Config": configs.get(idx, ""), # [新增] 將 config 值寫入
                    "Date": date,
                    "Sample Size": n,
                    "USL": usls.get(idx, ""),
                    "LSL": lsls.get(idx, ""),
...
if uploaded:
    ...
    if not df_cpk.empty:
        ...
        # [修改] 排序邏輯，確保 Config 欄位存在
        sort_cols = ["Station", "Dim No", "Date"]
