...
def _extract_data_for_cpk(df):
    # [修正] 搜尋邏輯：檢查前 10 欄中是否有精確匹配的關鍵字
    # 這樣可以避免 "SingleSide-USL" 被誤認為 "USL"
    def find_row_index(keywords):
        for i in range(min(60, len(df))):
            # 取得該列前 10 個 cell 的值，轉字串並正規化
            row_vals = [str(v).strip().lower() for v in df.iloc[i, :10]]
            # 檢查是否有任何 cell 的值 "精確等於" 關鍵字之一
