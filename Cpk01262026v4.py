...
def _extract_data_for_cpk(df):
    ...
    dim_row = find_row_index(["dim. no", "dim no", "dim.no", "dim. no."])
...
# Execution
# =========================
filename = "Rocky P2 Right IPQC dimension Data 26.1.24-N.xlsx"
...
for sheet in xls.sheet_names:
    ...
    yield_combined_counts.extend(process_yield_combined(station, df, ["SHD", "SHA", "SHC"], "SHD+SHA+SHC"))
    
    cpk_list.extend(process_cpk(station, df))
    cpk_config_list.extend(process_cpk_by_config(station, df))
    cpk_combined_list.extend(process_cpk_combined(station, df, ["SHD", "SHA", "SHC"], "SHD+SHA+SHC"))
...
if yield_config_counts:
    ...
    df_yield_agg["Yield Raw"] = df_yield_agg.apply(lambda r: r["OK"] / r["Total"] if r["Total"] > 0 else 0, axis=1)
...
if yield_combined_counts:
    ...
    df_yield_combined_agg["Yield Raw"] = df_yield_combined_agg.apply(lambda r: r["OK"] / r["Total"] if r["Total"] > 0 else 0, axis=1)

# Categorized Table Generator
def generate_categorized_table(df_cpk_data, df_yield_data):
    if df_cpk_data.empty: return pd.DataFrame()
    ...
    if df_for_stats.empty: return pd.DataFrame()
    ...
    for agg_type, cat_name in df_pivot.columns:
        if "Category 1" in cat_name: prefix = "Category1(X/Y/Rotation)"
        elif "Category 2" in cat_name: prefix = "Category2(height/ tilt)"
        else: prefix = cat_name
    ...
    cols = ['Station', 'Config', 'Sample Size', 'Yield',
...
if not df_cpk_config.empty:
    ...
    if not df_s.empty:
        ...
        cols_a = ['Station', 'Config', 'Sample Size', 'Yield', 'Min CPK Dim', 'Min CPK', 'Max CPK', 
                  'Dims CPK < 1.33', 'Dims CPK < 1.0',
                  'Category1(X/Y/Rotation) Dims CPK < 1.33', 'Category1(X/Y/Rotation) Dims CPK < 1.0',
                  'Category2(height/ tilt) Dims CPK < 1.33', 'Category2(height/ tilt) Dims CPK < 1.0']
...
# Export
output_filename = "IPQC_Report_Check.xlsx"
