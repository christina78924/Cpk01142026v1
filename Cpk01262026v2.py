...
def _extract_data_for_cpk(df):
    # [修正] 針對列的搜尋邏輯：檢查前 10 欄中是否有精確匹配的關鍵字
    def find_row_index(target_tokens):
        # target_tokens: list of lower-case strings
        for i in range(min(60, len(df))):
            # 檢查前 10 欄，避免漏掉
            row_vals = [str(v).strip().lower() for v in df.iloc[i, :10]]
            if any(t in row_vals for t in target_tokens):
                return i
        return -1

    dim_row = find_row_index(["dim. no", "dim no", "dim.no", "dim_no"])
    usl_row = find_row_index(["usl"])
    lsl_row = find_row_index(["lsl"])
...
def process_cpk_by_config(station, df):
    ...
    for config_val, g in data.groupby("Config"):
        for idx, dim in dim_cols.items():
            ...
            # 若為單邊公差，將另一邊設為 NaN 讓 calculate_cpk 忽略之
...
# [這部分的 process_cpk (By Date) 也要同步更新，雖然報表沒用到但為了一致性]
...
if uploaded:
    ...
    with tabs[0]: st.dataframe(df_yield, use_container_width=True)
    with tabs[1]: st.write(df_cpk.style.applymap(highlight_low_cpk, subset=["CPK"]))
    with tabs[2]: st.write(df_cpk_config.style.applymap(highlight_low_cpk, subset=["CPK"]))
    with tabs[3]: st.write(df_summary.style.applymap(highlight_low_cpk, subset=["Min CPK", "Max CPK"]))
    with tabs[4]: st.write(df_analysis.style.applymap(highlight_low_cpk, subset=["Min CPK", "Max CPK"]))
    with tabs[5]: st.write(df_categorized_wide.style.applymap(highlight_low_cpk, subset=[c for c in df_categorized_wide.columns if "Cpk" in c and "Dims" not in c]))
    with tabs[6]: st.write(df_combined_wide.style.applymap(highlight_low_cpk, subset=[c for c in df_combined_wide.columns if "Cpk" in c and "Dims" not in c]))
