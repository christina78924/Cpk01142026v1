...
if uploaded:
    ...
    if not df_cpk_config.empty:
        ...
        if not df_for_stats.empty:
            ...
            # [修改] 合併 Yield by Config 與 Sample Size
            if not df_yield_agg.empty:
                # 併入 Yield Raw 與 Total
                df_summary = pd.merge(df_summary, df_yield_agg[["Station", "Config", "Yield Raw", "Total"]], 
                                    on=["Station", "Config"], how="left")
                # 格式化良率
                df_summary["Yield"] = df_summary["Yield Raw"].apply(lambda x: f"{x:.2%}" if pd.notnull(x) else "")
                # [新增] 格式化 Sample Size (fillna(0) 防呆)
                df_summary["Sample Size"] = df_summary["Total"].fillna(0).astype(int)
            else:
                df_summary["Yield"] = ""
                df_summary["Sample Size"] = 0

            # [修改] 調整欄位順序: Sample Size 在 Yield 之前
            cols = ['Station', 'Config', 'Sample Size', 'Yield', 'Min CPK Dim', 'Min CPK', 'Max CPK']
            # 防呆：確保欄位存在才選取
    ...
    with tab4:
        st.write("統計每個站點與 Config 下的 Sample Size, Yield 及 CPK 極值")
