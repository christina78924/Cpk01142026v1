    ...
    if not df_cpk_config.empty:
        ...
        if not df_for_stats.empty:
            ...
            if not df_cat_source.empty:
                ...
                # 重新命名欄位 (Flatten MultiIndex columns)
                # 預期欄位: Category1(X/Y/Rotation) min Cpk, etc.
                new_columns = []
                ...
                # 合併 Sample Size, Yield (from yield_info), Failed lists (from failed_lists)
                # yield_info & failed_lists 已經有 Station, Config
    ...
    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
        ...
        def apply_cond_format(sheet_name, df, cols_to_check):
            ...
            for col_name in cols_to_check:
                ...
                # 規則順序很重要：先判斷嚴重的，或者使用 stop_if_true (xlsxwriter API 預設是依序套用)
                # 這裡設定區間
